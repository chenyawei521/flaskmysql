...
for k in range(1, 340):
    print ss.cell_value(k, 10)
    if ss.cell_value(k, 10) == 'NULL' or ss.cell_value(k, 10) == '':
        pass
    else:
        testin1 = str(ss.cell_value(k, 10))
        print testin1
        outputdata = []
        outputs = []
        ...
        ws.write(k, 13, geneid)
        ws.write(k, 12, phenoname)
        ws.write(k, 11, phenoid)
        ws.write(k, 14, genename)
        ws.write(k, 15, json.dumps(outputs))
        w.save('D:\ss.xls')
