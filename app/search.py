...
for k in range(1, 340):
    print ss.cell_value(k, 18)
    if ss.cell_value(k, 18) == 'NULL' or ss.cell_value(k, 18) == '':
        pass
    else:
        testin1 = str(ss.cell_value(k, 18))
        print testin1
        outputdata = []
        #find the rows which you want to select and write to a txt file
        for i in range(nrows):
            # cell_value = sh.cell_value(i, columnnum)
            cell_value = sh.cell_value(i, 5)
        ...
        ws.write(k, 21, geneid)
        ws.write(k, 20, phenoname)
        ws.write(k, 19, phenoid)
        ws.write(k, 22, genename)
        ws.write(k, 24, Inheritance)
        ws.write(k, 23, json.dumps(outputdata))
        w.save('D:\ss.xls')
